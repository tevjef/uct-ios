...
class OptionsViewController: UITableViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var universities = Array<University>() {
        didSet {
            ...
            let index = universities.index {
                $0.topicName == coreData.university!.topicName
            } ?? 0
            ...
            stopIndicator(activityIndicator)
        }
    }
    ...
    func loadData() {
        startIndicator(activityIndicator, { self.universities.count == 0 })
        ...
        appDelegate.dataRepo?.getUniversities({
            ...
            self?.stopIndicator(self?.activityIndicator)
        })
    }

    func setupViews() {
        navigationItem.title = "Search Options"
        activityIndicator = makeActivityIndicator(self.view)
    ...
    func setupUniversityPickerView() {
        ...
        let doneButton = UIBarButtonItem(
                ...
                target: self,
                action: #selector(OptionsViewController.doneUniversityPicker))
        ...
        let cancelButton = UIBarButtonItem(
                ...
                style: UIBarButtonItemStyle.plain,
                target: self,
                action: #selector(OptionsViewController.cancelUniversityPicker))
